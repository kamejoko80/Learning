...
void cropEye(const cv::Mat &img_u, const cv::Mat &et, const cv::Mat &face_2D, const cv::Mat hR, const Mat target,
			const cv::Mat &cam, cv::Mat &img_warped, cv::Mat &hrnew, cv::Size roiSize,
			 string save_path, string img_name, ofstream &log)
{
	float focal_new, distance_new;
	focal_new = 800.f;
	distance_new = 900.f;
    cv::Mat rotMat, cam_new, scaleMat;
	camPointTo(img, img_warped, rotMat, cam_new, scaleMat, roiSize, hR.col(0), et, cam, distance_new, focal_new);
	...
	cv::Mat new_face = warpMat * face_2D;
	for (int p = 0; p < 21; p++)
	{
		new_face.at<float>(0, p) /= new_face.at<float>(2, p);
		new_face.at<float>(1, p) /= new_face.at<float>(2, p);
	}
	...
	if (head_la < 80 && head_la > -80 && head_lo < 80 && head_lo > -80)
	{
		int width = 120;
		int height = 40;
		int mid_x = (new_face.at<float>(0, 16) + new_face.at<float>(0, 17)) /2;
		int mid_y = (new_face.at<float>(1, 16) + new_face.at<float>(1, 17)) /2;
		int x = mid_x - width/2;
		...
		log << img_name << " " << x << " " << y << " " << width << " " << height << " " << head_lo << " " << head_la << " " << eye_lo << " " << eye_la << endl;
		//char str[100];
		//sprintf(str, "lo: %.f; la: %.f", head_lo, head_la);
		//putText(img_warped, str, cv::Point(10,50), FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(0,255,0), 2);
		string head_path = save_path + "/head/";
		cv::imwrite(head_path + img_name, img_warped);
...
void normalizeData(const Mat &img_u, const Mat &face, const Mat &face_2D, const Mat target, const cv::Mat &cam,
                    const vector<float> &dist, string save_path, string img_name, ofstream &log)
{
    // undistort image
	...
	cropEye(img_u, mid_head.t(), face_2D, hR, target, cam, img_warped, hrnew, head_roiSize, save_path, img_name, log);
}
...
//#pragma omp parallel for
	for (int case_i = 0; case_i < cases.size(); case_i++)
	{
		...
		for (int cam_i = 0; cam_i < cases[case_i].camera_Idx.size(); cam_i++)
		{
			...
			// get face 2D
			Mat face_2D(3, 21, CV_32FC1);
			for (int p = 0; p < 21; p++)
			{
				face_2D.at<float>(0, p) = cases[case_i].pts[camId][p].x;
				face_2D.at<float>(1, p) = cases[case_i].pts[camId][p].y;
				face_2D.at<float>(2, p) = 1.f;
			}
			...
			if (re_pt.x > le_pt.x)
				normalizeData(img_u, face_cam, face_2D, target, cam, dist, save_path, img_name, log_file);
			else
