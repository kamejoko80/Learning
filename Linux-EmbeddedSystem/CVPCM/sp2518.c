...
//---------------------------------------------added by rpdzkj ivy  for camera power control 2015.11.19  行
//#include "sp2518_power_ctrl.h"
extern int is_sp2528_power_down;
...
#define RP5065_I2C_RETRY		3
//unsigned int io = CAMERA_PD0 ;
...
#define HERE printk("<%s>\n",__func__);
static int Pic_5M = 0; // 5M take picture 
static int Pic_2M = 0; // 3M take picture 
static int Pic_VGA = 0; // VGA take picture 
static int Pic_720P = 0;  // 720p video
...
#define SP2518_5MEG_WIDTH	2592
#define SP2518_5MEG_HEIGHT	1944
#define SP2518_MAX_WIDTH    SP2518_5MEG_WIDTH
#define SP2518_MAX_HEIGHT   SP2518_5MEG_HEIGHT
...
/* 2592x1944 */
static const struct sp2518_win_size sp2518_win_5meg = {
    .name     = "5MEG",
    .width    = SP2518_5MEG_WIDTH,
    .height   = SP2518_5MEG_HEIGHT,
 //   .win_regs = sp2518_uxga_regs,
    .frame_rate_array = frame_rate_uxga,
};
...
static int sp2518_init(struct v4l2_subdev *sd, u32 val)
{
	int ret = 0, err = 0, i = 0, count = 0;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	if(client->addr == 0x1f){
		...
		for (i = 0; i < HM5065_INIT_REG1S; i++) 
		{
			...
			if (err < 0)
			{
				...
				//added by rpdzkj ivy 2015.11.13
				if(count == 5){
						printk("failed to write hm5065_init_reg1 data!!!\n");
						break;
				}
				count=0;
		...
		for (i = 0; i < HM5065_INIT_REG2S; i++) 
		{
			...
			if (err < 0)
			{
				...
				//added by rpdzkj ivy 2015.11.13
				if(count == 5){
						printk("failed to write hm5065_init_reg2 data!!!\n");
						break;
				}
				count=0;
			}
		...
		for (i = 0; i < HM5065_INIT_REG3S; i++) 
		{
			...
			if (err < 0)
			{
				...
				//added by rpdzkj ivy 2015.11.13
				if(count == 5){
						printk("failed to write hm5065_init_reg3 data!!!\n");
						break;
				}
				count=0;
			}
...
//							printk("%s--------222[%d]\n",__func__,count);							
						...
						for (i = 0; i < HM5065_5M_REGS; i++) 
							{
								...
								{
									v4l_info(client, "[3]: %d  init register [hm5065] 0x%x%x set failed\n",
											i, hm5065_5M_reg[i][0], hm5065_5M_reg[i][1]);
		...
		printk(" \44444444444444444444444444444444444444\n");
...
#if 1  
	if(is_sp2528_power_down){
		sp2518_init(sd, enable);
		is_sp2528_power_down = 0;
		}
#endif
				...
				if(Pic_5M == 1){
					...
					for (i = 0; i < HM5065_5M_REGS; i++) 
					{
						...
						if (err < 0)
						{
							...
							//added by rpdzkj ivy 2015.11.13
							if(count == 5){
								printk("failed to write hm5065_5M_reg data!!!\n");
								break;
							}
							count=0;
						}
					...
					for (i = 0; i < HM5065_VGA_PREVIEW; i++) 
					{
						...
						{
							v4l_info(client, "[3]: %d  init register [hm5065] 0x%x%x set failed\n",
									i, hm5065_vga_preview[i][0], hm5065_vga_preview[i][1]);
							...
							//added by rpdzkj ivy 2015.11.13
							if(count == 5){
								printk("failed to write hm5065_vga_preview data!!!\n");
								break;
							}
							count=0;
						}
					...
					for (i = 0; i < HM5065_2M_REGS; i++) 
					{
						...
						{
							v4l_info(client, "[3]: %d  init register [hm5065] 0x%x%x set failed\n",
									i, hm5065_2M_reg[i][0], hm5065_2M_reg[i][1]);
							...
							//added by rpdzkj ivy 2015.11.13
							if(count == 5){
								printk("failed to write hm5065_2M_reg data!!!\n");
								break;
							}
							count=0;
						}
					...
					for (i = 0; i < HM5065_720P_REGS; i++) 
					{
						...
						{
							v4l_info(client, "[3]: %d  init register [hm5065] 0x%x%x set failed\n",
									i, hm5065_720p_reg[i][0], hm5065_720p_reg[i][1]);
	
							do{
								err = rp5065_i2c_write(sd, hm5065_720p_reg[i],sizeof(hm5065_720p_reg[i]));
								count++;
							}while(err<0 && count<5);
							//added by rpdzkj ivy 2015.11.13
							if(count == 5){
								printk("failed to write hm5065_720p_reg data!!!\n");
								break;
							}
							count=0;
						}
        ...
        struct v4l2_subdev_format *fmt)
{
	...
	if(mf->width == 2592){	// 2592 x 1944
		Pic_5M = 1;
		Pic_VGA = 0;
		Pic_2M = 0;
		Pic_720P = 0;
	}
	else if(mf->width == 640){ // 640 x 480
		Pic_5M = 0;
		Pic_VGA = 1;
		Pic_2M = 0;
		Pic_720P = 0;
	}else if(mf->width == 1600){
		Pic_5M = 0;
		Pic_VGA = 0;
		Pic_2M = 1;
		Pic_720P = 0;
